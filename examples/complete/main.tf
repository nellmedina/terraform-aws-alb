...
module "alb" {
  ...
  subnets = ["subnet-04676ca5151426d2b", "subnet-053b91915bbcffd18"]
...
module "s3_lb_log" {
  source                = "git::https://github.com/tmknom/terraform-aws-s3-lb-log.git"
  name                  = "s3-lb-log-alb-${var.alb_name}-${data.aws_caller_identity.current.account_id}"
  logging_target_bucket = "${module.s3_access_log.s3_bucket_id}"
  force_destroy         = true
}

module "s3_access_log" {
  source        = "git::https://github.com/tmknom/terraform-aws-s3-access-log.git"
  name          = "s3-access-log-alb-${var.alb_name}-${data.aws_caller_identity.current.account_id}"
  force_destroy = true
}
...
# module "vpc" {
#   source                    = "git::https://github.com/tmknom/terraform-aws-vpc.git"
